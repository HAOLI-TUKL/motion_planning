    ...
    for t = 0:0.01:1
        ...
        for i = 0:n_order
            basis_p = nchoosek(n_order, i) * t^i * (1-t)^(n_order-i);
             x_pos(idx) = poly_coef_x((k-1)*(n_order+1)+i+1)*basis_p + x_pos(idx);
             y_pos(idx) = poly_coef_y((k-1)*(n_order+1)+i+1)*basis_p + y_pos(idx);
        end
        idx = idx + 1;
    end
end
...
plot(x_pos, y_pos);
for k=1:n_seg
    scatter( poly_coef_x((k-1)*8+1:k*8),  poly_coef_y((k-1)*8+1:k*8), 15,'filled' );
end
...
function poly_coef = MinimumSnapCorridorBezierSolver(axis, waypoints, corridor, ts, n_seg, n_order, v_max, a_max)
    ...
    if axis == 1
       for seg = 1:n_seg
           corridor_range(seg,1) = corridor(1,seg) - corridor(3,seg);  
           corridor_range(seg,2) = corridor(1,seg) + corridor(3,seg); 
       end
    else
       for seg = 1:n_seg
           corridor_range(seg,1) = corridor(2,seg) - corridor(4,seg);  
           corridor_range(seg,2) = corridor(2,seg) + corridor(4,seg); 
       end
    end
