...
def dynamic_programming():
    ...
    stop_time = time()
    print("time cost :" )
    print(str(stop_time-start_time) + "s")
    plt.figure()
...
def rtdp():
    ...
    while bellman_error > 0.0001:
        ...
        for key in graph.keys():
            state = graph[key]
            if state.px == START_LINE[idx][0] and state.py == START_LINE[idx][1]:
                start_state = state 
                break
        bellman_error = update(start_state,bellman_error)
        bellman_error_list.append(bellman_error)
        print("{}th iteration: {}".format(itr_num, bellman_error))
    stop_time = time()
    print("time cost :" )
...
def update(state,bellman_error):
    if state.is_goal:
        return bellman_error
    else:
        value_uk = []
        min_state = graph[state.next_prob_9[0]]
        for child_idx in range(len(ACTION_SPACE)):
            child_key_9 = state.next_prob_9[child_idx]
            child_9 = graph[child_key_9]
            if child_9.g_value < min_state.g_value:
                min_state = child_9
            child_key_1 = state.next_prob_1[child_idx]
            child_1 = graph[child_key_1]
            if child_1.g_value < min_state.g_value:
                min_state = child_1
            expected_cost_uk = 0.9 * (1 + child_9.g_value) + 0.1 * (1 + child_1.g_value)
            value_uk.append(expected_cost_uk)
        current_value = min(value_uk)
        bellman_error += np.linalg.norm(state.g_value - current_value)
        state.g_value = min(value_uk) 
        bellman_error = update(min_state,bellman_error)
        return bellman_error
