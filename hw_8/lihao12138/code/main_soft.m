...
%    F(1,2*K+1:3*K) = w1*(Bpz'*Tpy-z'*Tpz);
   ...
   H = blkdiag(w1*Tpx'*Tpx+Tvx'*Tvx+Tax'*Tax+eye(K), ...
       w1*Tpy'*Tpy+Tvy'*Tvy+Tay'*Tay+eye(K),...
       w1*Tpz'*Tpz+Tvz'*Tvz+Taz'*Taz+eye(K),w2*eye(K),w3*eye(K));
   F = zeros(1,K*5); 
   F(1,1:K) = w1*(Bpx'*Tpx-x'*Tpx)+Bvx'*Tvx+Bax'*Tax;
   F(1,K+1:2*K) = w1*(Bpy'*Tpy-y'*Tpy)+Bvy'*Tvy+Bay'*Tay;
   F(1,2*K+1:3*K) = w1*(Bpz'*Tpy-z'*Tpz)+Bvz'*Tvz+Baz'*Taz;
